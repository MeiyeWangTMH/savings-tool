...
folder='data/'+SiteName+ '/' + date +'/Output/'
year=2021
user_filename_list=['Charging Data - '+ SiteName+ ' - ' + date+ '.xlsx']
monthStart=10
monthEnd=10
...
for month in monthRange:
    ...
    output_month=output_df[output_df['month']==month]
    output_month= output_month.drop(columns='month')
    output_month.to_csv('Data Output/'+str(date) +'_TheMobilityHouse_Interval(' +str(year)+'-0'+str(month)+'_CRT-2019-0052_session.csv',index=False)
